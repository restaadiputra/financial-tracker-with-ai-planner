...
export function RecurringForm({
  ...
  // Day-of-month is 1–31: strip non-digits and clamp on input.
  function handleDayOfMonthChange(raw: string) {
    const digits = raw.replace(/\D/g, '').slice(0, 2);
    if (digits === '') return setDayOfMonth('');
    setDayOfMonth(String(Math.min(Math.max(Number(digits), 1), 31)));
  }
    ...
    <Sheet open onClose={onCancel} variant="modal" labelledBy={titleId}>
      <form onSubmit={handleSubmit} className="flex flex-col">
        <div className="flex flex-col gap-4 px-5 pb-5 pt-4">
            ...
            {(frequency === 'monthly' || frequency === 'yearly') && (
              <label className="flex w-28 flex-col gap-1 text-label">
                ...
                <input
                  ...
                  aria-label="Day of month, 1 to 31"
                  className={`${fieldClass} tabular-nums`}
                />
              </label>
            )}
          ...
          <label className="-mx-1 flex min-h-11 items-center gap-2 px-1 py-2.5 text-label">
            <input
              type="checkbox"
              checked={isActive}
