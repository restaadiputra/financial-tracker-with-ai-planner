...
export const secondaryButton =
  'min-h-11 rounded-control border border-border px-4 py-2.5 text-label font-medium text-muted transition-colors duration-150 ease-out-quart hover:border-accent hover:text-foreground active:bg-surface-hover focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent';
...
// Tappable area is guaranteed via `min-h-11` (44px) without bumping visible
// padding, so text-only inline links ("Edit", "Cancel", "Switch profile")
// stay visually compact — bumping padding instead would make these list-row
// actions look noticeably bulkier than the rest of the system (PRD touch-
// target pass). Content centers in the extra height via flex.
export const ghostButton =
  'inline-flex min-h-11 items-center justify-center rounded-control px-2 py-1.5 text-label text-muted transition-colors duration-150 ease-out-quart hover:bg-surface-hover hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent';
